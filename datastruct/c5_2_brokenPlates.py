...
class LinkedList:
    ...
    def __getitem__(self, index):
        if self.size == 0:
            raise IndexError 
        while index < 0 and self.is_not_empty():
            index += self.size
        if index >= self.size:
            raise IndexError
        return self.at_node(index).data
...
class Plate:
    def __init__(self,weight = None, freq = None):
        self.weight = weight
        self.freq= freq

if __name__ == "__main__":
    input_data = input("Enter Input : ").split(",")
    plates_stack = LinkedList()
    for data in input_data:
        data_split = data.split()
        plate = Plate(int(data_split[0]),int(data_split[1]))
        while plates_stack.is_not_empty() and plates_stack[-1].weight < plate.weight :
            broken_plate = plates_stack.pop()
            print(broken_plate.freq)
        plates_stack.append(plate)