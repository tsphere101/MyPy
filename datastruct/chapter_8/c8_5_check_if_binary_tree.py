...
def check_binary_search_tree_(root, L=None):
    if L is None:
        L = []
    ...
    if root.left and root.right:
        ...
        if len(set(L)) != len(L):
            return False
        return check_binary_search_tree_(root.left, L) and check_binary_search_tree_(root.right, L) and (0 <= root.data <= 100)
    ...
    if root.left and not root.right:
        ...
        if len(set(L)) != len(L):
            return False
        return check_binary_search_tree_(root.left, L) and (0 <= root.data <= 100)
    ...
    if not root.left and root.right:
        ...
        if len(set(L)) != len(L):
            return False
        return check_binary_search_tree_(root.right, L) and (0 <= root.data <= 100)

    if not root.left and not root.right:
        # Base case
        if len(set(L)) != len(L):
            return False
        return True and (0 <= root.data <= 100)
